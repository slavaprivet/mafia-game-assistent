...
async def _process_task(user_id: int, task_id: int, text: str, status_msg: Message, model_info: dict):
    try:
        ...
        system_prompt = (
            ...
            "Структура NPC объекта (строго эти поля, ничего лишнего):\n"
            "  { r: 36.5, c: 36.5, tr: 36.5, tc: 36.5, speed: 0.8,\n"
            "    ang: 0, walkPhase: 0, idleUntil: 0, hp: 60, max_hp: 60,\n"
            "    name: 'Имя',  // необязательно — для именных NPC\n"
            "    look: { gender: 0, skin: 2, body: 1, face: 3, hair: 4, hat: 1 } }\n"
            "Важные переменные:\n"
            "  BURJ_POS = {r:36, c:36}  — здание Burj Mafia в центре города\n"
            "  MAP_ROWS, MAP_COLS — размер карты\n"
            "  NPCS — массив всех NPC\n"
            "  pickNpcWaypoint(npc) — задаёт маршрут NPC\n"
            "Функция спавна NPC вручную:\n"
            "  const npc = { r: X, c: Y, tr: X, tc: Y, speed: 0.6,\n"
            "    ang:0, walkPhase:0, idleUntil:0, hp:60, max_hp:60,\n"
            "    look:{gender:0, skin:1, body:1, face:2, hair:3, hat:0} };\n"
            "  pickNpcWaypoint(npc); NPCS.push(npc);\n\n"
            "=== ФОРМАТЫ ОТВЕТА ===\n"
            "ФОРМАТ А — добавление кода ВНУТРЬ функции:\n"
            "  1 строка: что добавлено\n"
            "  ФУНКЦИЯ: initNpcs\n"
            "  СТАЛО:\n  ```\n  // ТОЛЬКО новые строки для вставки внутрь функции\n"
            "  // НЕ пиши объявление функции! Только тело.\n"
            "  const npc = {...}; pickNpcWaypoint(npc); NPCS.push(npc);\n  ```\n"
            "  Файл: world.html\n\n"
            "ФОРМАТ Б — замена существующего кода:\n"
            "  1 строка: что изменено\n"
            "  БЫЛО:\n  ```\n  точная копия строк\n  ```\n"
            "  СТАЛО:\n  ```\n  новый код\n  ```\n"
            "  Файл: world.html\n\n"
            "КРИТИЧЕСКИЕ ПРАВИЛА:\n"
            "• В СТАЛО: при ФУНКЦИЯ: — пиши ТОЛЬКО код для вставки, БЕЗ объявления функции\n"
