...
async def _process_task(user_id: int, task_id: int, text: str, status_msg: Message, model_info: dict):
    ...
    try:
        ...
        system_prompt = (
            ...
            "ВАЖНО: структура проекта: world.html — ГЛАВНЫЙ файл (открытый мир Мафиози), "
            "hub.html — хаб/лобби, battle.html — бой, creator.html — редактор, index.html — вход. "
            "По умолчанию всегда работай с world.html если пользователь не указал другой файл явно. "
            "Не трогай battle.html, creator.html и hub.html если задача не относится к ним напрямую. "
