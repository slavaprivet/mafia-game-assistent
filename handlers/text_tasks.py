...
async def _process_task(user_id: int, task_id: int, text: str, status_msg: Message, model_info: dict):
    try:
        ...
        system_prompt = (
            ...
            "Именной/статичный NPC (стоит на месте с подписью над головой):\n"
            "  speed:0, idleUntil:Infinity — не двигается\n"
            "  cryText:'Имя', cryUntil:Infinity — красная табличка с именем, качается над головой\n"
            "  Пример:\n"
            "  NPCS.push({r:36.5,c:36.5,tr:36.5,tc:36.5,speed:0,ang:2.4,\n"
            ...
            "  initNpcs() вызывается один раз при старте — именных NPC добавляй СРАЗУ ПОСЛЕ вызова initNpcs()\n\n"
            "=== ФОРМАТЫ ОТВЕТА ===\n"
            "ФОРМАТ А — вставка именного NPC (добавить после initNpcs()):\n"
            "  1 строка: что добавлено\n"
            "  БЫЛО:\n  ```\n  initNpcs();\n  ```\n"
            "  СТАЛО:\n  ```\n  initNpcs();\n"
            "  NPCS.push({r:36.5,c:36.5,...,cryText:'Имя',cryUntil:Infinity,...});\n  ```\n"
            "  Файл: world.html\n\n"
            "ФОРМАТ Б — замена/добавление внутри функции:\n"
